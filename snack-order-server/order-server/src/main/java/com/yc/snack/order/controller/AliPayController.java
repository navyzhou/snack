...
import com.yc.snack.order.config.AlipayConfig;
...
public class AliPayController{
	@Autowired
	private IOrderInfoService orerInfoService;

	@RequestMapping("/pay/{out_trade_no}/{total_amount}")
	public void pay(HttpServletResponse response,@PathVariable String out_trade_no, @PathVariable String total_amount) {
		//获得初始化的AlipayClient
	...
	public void callBackNotify(HttpServletRequest request, HttpServletResponse response) throws IOException, AlipayApiException {
		...
		// 修改订单状态
		orerInfoService.update(out_trade_no, 2);
		if (signVerified) {//验证成功
			...
			out.println("<script>location.href='/order/order.html'</script>");
	...
	public void callbackReturn(HttpServletRequest request, HttpServletResponse response) throws AlipayApiException, IOException {
		...
		PrintWriter out = response.getWriter();
		String out_trade_no = new String(request.getParameter("out_trade_no").getBytes("ISO-8859-1"), "UTF-8");
		orerInfoService.update(out_trade_no, 2); // 修改订单状态
		if (signVerified) {
			//商户订单号
